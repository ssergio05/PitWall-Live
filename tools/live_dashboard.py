...
if not paths:
    ...
    exit()
csv_file = paths[0]
print(f"📡 Connecting to Live F1 Pit Wall... Reading: {csv_file}")
...
def update_graph(frame):
    try:
        ...
        # 25-second window (1500 data points at 60Hz) for full corner context
        df = df.tail(1500)
        ...
        # --- PANEL 1: Speed ---
        ax1.plot(df['SessionTime'], df['Speed'], color='#00ffff', linewidth=2)
        ax1.set_ylim(0, 360)
        ax1.set_ylabel("Speed (Km/h)", fontsize=10, fontweight='bold')
        ax1.grid(True, alpha=0.2)

        # --- PANEL 2: RPM and Gear ---
        ax2.plot(df['SessionTime'], df['RPM'], color='#ffa500', linewidth=1.5)
        ax2.set_ylim(8000, 12500)
        ax2.set_ylabel("RPM", color='#ffa500', fontsize=10, fontweight='bold')
        
        # Overlay gear (stepped) on the right axis
        ax2_gear.step(df['SessionTime'], df['Gear'], color='white', linestyle='--', linewidth=1.5)
        ...
        # --- PANEL 3: Pedals ---
        ax3.plot(df['SessionTime'], df['Throttle'], color='#00ff00', linewidth=1.5, label='Throttle')
        ax3.plot(df['SessionTime'], df['Brake'], color='#ff0000', linewidth=1.5, label='Brake')
        ...
        ax4.plot(df['SessionTime'], df['Steer'], color='#ff00ff', linewidth=1.5)
        ax4.axhline(0, color='gray', linewidth=1, linestyle='--') # Center reference line
        ax4.set_ylim(-100, 100)
        ax4.set_ylabel("Steering", fontsize=10, fontweight='bold')
        ax4.set_xlabel("Session Time (s)", fontsize=10, fontweight='bold')
        ax4.grid(True, alpha=0.2)

        # Adjust layout for perfect margins
        plt.tight_layout()

    except Exception:
        # Ignore errors if the file is briefly locked by the C++ writer
        pass
